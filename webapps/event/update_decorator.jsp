...
<%
	...
	PreparedStatement ps = con.prepareStatement("select * from decorator where decorator_id=?");
	ps.setInt(1,did);
...
<tr>
	<td>
		<form method='post' action='update_decorator1.jsp' name='reg' onsubmit='return validate()'>
		<table width='100%' height='100%' class='admin'>
		<tr>
			<td align='right' class='text'><b>Decorator ID:</b></td>
			<td><input type='text' name='did' class='text' required onfocus='change(this)' onblur='unchange(this)' value=<%=rs.getInt(1)%> readOnly></td>
		</tr>
		<tr>
			<td align='right' class='text'><b>Decorator Name:</b></td>
			<td><input type='text' name='dname' class='text' required onfocus='change(this)' onblur='unchange(this)' value='<%=rs.getString(2)%>'></td>
		</tr>
		<tr>
